...
```
🌐 (web) → DOWNLOAD [params] → (csv) → INGEST [params] → (Postgres) 🐘
```

We have now separated our pipeline into a `DOWNLOAD` script that takes params like `url` and a `INGEST` script that takes various params like `user`, `password`, `host` etc. 

we can also include a **Makefile** so that we can inform it that the `INGEST` script depends on the `DOWNLOAD` script

> **In this lesson we will create a more complex pipeline aka `Data Workflow`:**

<img src="image.png" alt="complex pipeline" title="hello" width="500">

we will use **Parquet** which is a [columnar storage datafile format](https://parquet.apache.org/) which is more efficient than **CSV**.
...
The steps in the boxes are our ***jobs*** and the objects in between are the jobs' outputs, which behave as ***dependencies*** for other jobs. Each job may have its own set of ***parameters*** and there may also be global parameters which are the same for all of the jobs.
...
The tool we will focus on in this course is **[Apache Airflow](https://airflow.apache.org/)**, but there are many others such as Luigi, **Prefect**, Argo, etc. **Makefile** is also an example but it has localscope (ie. local machine) and is used for smaller workflows.f

They help us in dealing with errors, edge cases, steps to retry running the script in case of error etc.
...
* A **webserver** which seves as the GUI availabe on port `8080`
...
> **One operator = One task and operators are Atomic!**
